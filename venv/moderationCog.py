...
import discord
import asyncio
import AutoPilot, actionLogCog,funstuffCog
import time

isCog = True
isEnabled = True

class AnModUtilityModule(commands.Cog):
...
class AnModerationModule(commands.Cog):
    def __init__(self, bot):
        self.client = bot
    ...
    async def userinfo(self, context):
        ...
        if status == "OFFLINE":
            try:
                lastonline = time.ctime(AutoPilot.ServerSettings[str(guildID)]['ActivityTable'][str(user.id)]["RecentStats"]["lastMsgTime"])
            except KeyError:
                lastonline = "N/A"
        else:
            lastonline = "Now"
