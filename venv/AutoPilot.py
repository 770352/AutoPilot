...
import asyncio
import time, json
import AutoPilot, rankingsCog, musicPlayerCog, moderationCog, actionLogCog, funstuffCog, messageFiltercog
import sys, inspect, types, psutil, systemUtilitys
...
yellowSquare = "https://media.discordapp.net/attachments/515326457652707338/723986430421893160/adidas-adicolor-yellow-orange-square-shape-s-png-clip-art.png?width=668&height=587"
redSquare = "https://media.discordapp.net/attachments/515326457652707338/723982767326625832/CCS400RD.png?width=587&height=587"
...
def loadCogs(client,reloadName=None):
    for name, val in globals().items():
        if isinstance(val, types.ModuleType):
            try:
                if val.isCog:
                    loadCog(val, client, reloadName=reloadName)
            except AttributeError:
                #print("Not cog" + str(val))
                pass

def loadCog(module,client,reloadName=None):
    global cogs
    load = True
    for name, obj in inspect.getmembers(module):
        if inspect.isclass(obj):
            if reloadName:
                if obj.__name__ != reloadName:
                    load = False
                else:
                    load = True
            if str(obj.__name__).startswith("An") and load:
                if module.isEnabled:
                    try:
                        client.add_cog(obj(client))
                        print("Loaded Cog: " + str(obj.__name__) + "; From Module: " + str(module.__name__))
                        cogs[str(obj.__name__)] = {"Running":"No Problems"}
                    except Exception as e:
                        print("Failed To Load Cog: " + str(obj.__name__) + "; From Module: " + str(module.__name__) + "\nReason: " + str(e))
                        cogs[str(obj.__name__)] = {"Failed To Load":str(e)}
                else:
                    print("Found Cog: " + str(obj.__name__) + "; From Module: " + str(module.__name__))
                    cogs[str(obj.__name__)] = {"Disabled": "Not Loaded"}
                time.sleep(0.25)
...
class ManagmentModule(commands.Cog):
    def __init__(self, bot):
        self.client = bot
        self.modCog = moderationCog.AnModUtilityModule(self.client)
    ...
    async def invite(self,context):
        await context.send("https://discord.com/api/oauth2/authorize?client_id=514999044444127233&permissions=8&redirect_uri=https%3A%2F%2Fdiscord.com&scope=bot")
